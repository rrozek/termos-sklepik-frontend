...
function KidsPage() {
  ...
  useEffect(() => {
    const fetchKids = async () => {
      ...
      try {
        ...
        setError(errorInfo.message || 'No kids found. You can add your first kid below.');
        setKids([]);
      } finally {
        setIsLoading(false);
      }
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      if (response.success) {
        // Extract the kid data using the utility function
        const newKid = extractResponseData<Kid>(response);

        // Create a sanitized version for parents
        let sanitizedKid = newKid;
        if (user?.role === UserRole.PARENT) {
          sanitizedKid = {
            ...newKid,
            rfid_token: []
        ...
        setKids((prev) => [...prev, sanitizedKid]);
        setDialogOpen(false);
        form.reset();
        toast({
          title: 'Success',
          description: response.message || 'Kid added successfully.',
        });
      }
    } catch (error) {
      ...
      toast({
        title: 'Error',
        description: errorInfo.message || 'Failed to create kid. Please try again.',
        variant: 'destructive'
      });
    }
  };
